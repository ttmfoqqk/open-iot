...
class ReservationMenuHelper

	Dim selectSQL

	private sub Class_Initialize()
		selectSQL = " SELECT ROW_NUMBER() OVER (Order by [OrderNo] ASC,[No] DESC) AS [RowNum],count(*) over () as [tcount],* FROM [ReservationMenu] "
	end sub
	...
	public function Insert(ByRef obj)
		...
		strSQL= " Insert into [ReservationMenu] ([Location],[Name],[OrderNo]) " & _
		" Values (?,?,?); " & _
		...
		if DbAddParameters(objCommand, Array(obj.Location,obj.Name,obj.OrderNo)) then
	...
	public function SelectAll(objs,pageNo,rows)
		...
		if objs.Location <> "" then
			whereSql = whereSql & " and Location = @Location "
		end if

		selectSQL = "" &_
		" SET NOCOUNT ON;  " &_
		" DECLARE @Name VARCHAR(200) , @Location INT; " &_
		" SET @Name = ?; " &_
		" SET @Location = ?; " &_
		...
		With objCommand
			...
			.Parameters.Append .CreateParameter( "@Location" ,adVarChar , adParamInput, 200 , objs.Location )
		End With
	...
	public function Update(obj)
		Dim strSQL
		strSQL= "Update [ReservationMenu] set [Location] = ?, [Name] = ?, [OrderNo] = ? Where No = ? "
		set objCommand=Server.CreateObject("ADODB.command") 
		objCommand.ActiveConnection=DbOpenConnection()
		objCommand.NamedParameters = False
		objCommand.CommandText = strSQL
		...
		if DbAddParameters(objCommand, Array(obj.Location,obj.Name,obj.OrderNo,obj.No)) then
			objCommand.Execute
			Update = true
		Else
			Update = false
 	...
 	public function Delete(No)
		...
		" DELETE FROM [ReservationMenu] WHERE No in(SELECT T_INT FROM @T); "
