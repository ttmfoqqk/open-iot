...
class AdBannerHelper
	...
	private sub Class_Initialize()
		selectSQL = " SELECT ROW_NUMBER() OVER (order by [OrderNo] ASC , [No] DESC) AS [RowNum],count(*) over () as [tcount], * FROM [AdBanner] "
	end sub
	...
	public function Insert(ByRef obj)
		...
		strSQL=   " Insert into [dbo].[AdBanner] ( [Position],[Name],[Url],[Image],[OrderNo] )" & _
		" Values (?,?,?,?,?); " & _
		...
		if DbAddParameters(objCommand, Array(obj.Position,obj.Name,obj.Url,obj.Image,obj.OrderNo)) then
	...
	public function Update(obj)
		Dim strSQL
		strSQL= "Update [AdBanner] set [Position]=?,[Name]=?,[Url]=?,[Image]=?,[OrderNo]=? Where [No] = ? "
		set objCommand=Server.CreateObject("ADODB.command") 
		objCommand.ActiveConnection=DbOpenConnection()
		objCommand.NamedParameters = False
		objCommand.CommandText = strSQL
		...
		if DbAddParameters(objCommand, Array(obj.Position,obj.Name,obj.Url,obj.Image,obj.OrderNo,obj.No)) then
			objCommand.Execute
			Update = true
		Else
			Update = false
	...
	public function Delete(No)
		...
		" DELETE FROM [AdBanner] WHERE No in(SELECT T_INT FROM @T); "
