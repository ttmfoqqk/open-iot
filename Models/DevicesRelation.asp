...
class DevicesRelationHelper
	...
	private sub Class_Initialize()
		...
		" FROM [DevicesRelation] AS A " &_
		" INNER JOIN [Apps] AS B " &_
	...
	public function Insert(ByRef obj)
		...
		strSQL= " Insert into [DevicesRelation] (ParentNo,ProductNo)" & _
		" Values (?,?); " & _
	...
	public function SelectAll(No)
		...
		" INSERT INTO @T(T_INT) select [ProductNo] from [DevicesRelation] where [ParentNo]=@No " &_
		" INSERT INTO @T(T_INT) select [ParentNo] from [AppsRelation] where [ProductNo]=@No " &_
		
		" SELECT " &_
			"  0 AS [No] " &_
			" ,@No AS [ParentNo] " &_
			" ,[Apps].No AS [ProductNo] " &_
			" ,[Apps].Name AS ProductName " &_
			" ,[Apps].Contents1 AS ProductContents " &_
			" ,[Apps].Images1 " &_
			" ,[Apps].Images2 " &_
			" ,[Apps].Images3 " &_
			" ,[Apps].Images4 " &_
		...
		" WHERE [Apps].[State] = 0 " &_
		" AND [User].[DelFg] = 0 " &_
		" AND [Apps].[No] IN( select T_INT from @T group by T_INT ) " &_
		" ORDER BY [Apps].No DESC "
	...
	public function SelectByField(fieldName, value)
 	...
 	public function Delete(No)
		Dim strSQL
		strSQL= "DELETE FROM [DevicesRelation] WHERE No = ? "
