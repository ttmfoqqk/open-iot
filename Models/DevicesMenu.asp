...
class DevicesMenuHelper

	Dim selectSQL

	private sub Class_Initialize()
		selectSQL = " SELECT ROW_NUMBER() OVER (Order by [OrderNo] ASC,[No] DESC) AS [RowNum],count(*) over () as [tcount],* FROM [DevicesMenu] "
	end sub
	...
	public function Insert(ByRef obj)
		...
		strSQL= " Insert into [DevicesMenu] (Name,[OrderNo]) " & _
		" Values (?,?); " & _
	...
	public function Update(obj)
		Dim strSQL
		strSQL= "Update [DevicesMenu] set [Name] = ?, [OrderNo] = ? Where No = ? "
		set objCommand=Server.CreateObject("ADODB.command") 
		objCommand.ActiveConnection=DbOpenConnection()
		objCommand.NamedParameters = False
		objCommand.CommandText = strSQL
 	...
 	public function Delete(No)
		...
		" DELETE FROM [DevicesMenu] WHERE No in(SELECT T_INT FROM @T); "
