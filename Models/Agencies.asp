...
class Agencie
	...
	public property let Images(val)
		mImages = val
	end property
...
class AgencieHelper
	...
	private sub Class_Initialize()
		selectSQL = " SELECT ROW_NUMBER() OVER (order by [OrderNo] ASC , [No] DESC) AS [RowNum],count(*) over () as [tcount], * FROM [Agencie] "
	end sub
	...
	public function Insert(ByRef obj)
		...
		strSQL=   " Insert into [dbo].[Agencie] ( [Name],[Url],[Images],[OrderNo] )" & _
		" Values (?,?,?,?); " & _
		...
		if DbAddParameters(objCommand, Array(obj.Name,obj.Url,obj.Images,obj.OrderNo)) then
	...
	public function Update(obj)
		Dim strSQL
		strSQL= "Update [Agencie] set [Name]=?,[Url]=?,[Images]=?,[OrderNo]=? Where [No] = ? "
		set objCommand=Server.CreateObject("ADODB.command") 
		objCommand.ActiveConnection=DbOpenConnection()
		objCommand.NamedParameters = False
		objCommand.CommandText = strSQL
		...
		if DbAddParameters(objCommand, Array(obj.Name,obj.Url,obj.Images,obj.OrderNo,obj.No)) then
			objCommand.Execute
			Update = true
		Else
			Update = false
	...
	public function Delete(No)
		...
		" DELETE FROM [Agencie] WHERE No in(SELECT T_INT FROM @T); "
