...
class DevicesHelper
	...
	private sub Class_Initialize()
		selectSQL = " SELECT ROW_NUMBER() OVER (order by [No] DESC) AS [RowNum],count(*) over () as [tcount],* FROM [Devices] "
	end sub
	...
	public function Insert(ByRef obj)
		...
		strSQL= " Insert into [Devices] (UserNo,MenuNo,Name,Contents1,Contents2,Contents3,InDate,AdminNo,Images1,Images2,Images3,Images4,DelFg,State,ImagesList)" & _
		" Values (?,?,?,?,?,?,GETDATE(),?,?,?,?,?,0,?,?); " & _
	...
	public function SelectAll(objs,pageNo,rows)
		...
		if objs.MenuNo <> "" then
			whereSql = whereSql & " and [Devices].MenuNo = @MenuNo "
		end if
		if objs.Name <> "" then
			whereSql = whereSql & " and [Devices].Name like '%'+ @Name+ '%' "
		end if
		if objs.Sdate <> "" then
			whereSql = whereSql & " and CONVERT(VARCHAR,[Devices].[InDate],23) >= @Sdate "
		end if
		if objs.Edate <> "" then
			whereSql = whereSql & " and CONVERT(VARCHAR,[Devices].[InDate],23) <= @Edate "
		end if
		
		if objs.State <> "" then
			whereSql = whereSql & " and [Devices].State = @State "
		end if
		...
		" WITH LIST AS ( " &_
				" SELECT " &_
				"  ROW_NUMBER() OVER (order by [Devices].[No] ASC) AS RowNum " &_
				" ,count(*) over () as [tcount] " &_
				" ,[Devices].* " &_
				" ,[DevicesMenu].Name AS MenuName " &_
				" ,( CASE [Devices].AdminNo WHEN 0 THEN [User].[Id] ELSE [Admin].[Id] END ) AS UserId " &_
				" ,( CASE [Devices].AdminNo WHEN 0 THEN [User].[Name] ELSE [Admin].[Name] END ) AS UserName " &_
			" FROM [Devices] AS [Devices] " &_
			" INNER JOIN [User] AS [User] ON([Devices].UserNo = [User].No) " &_
			" LEFT JOIN [DevicesMenu] AS [DevicesMenu] ON([Devices].MenuNo = [DevicesMenu].No) " &_
			" LEFT JOIN [Admin] AS [Admin] ON([Devices].AdminNo = [Admin].No) " &_
			" WHERE [Devices].[DelFg] = 0 AND [User].[DelFg] = 0 " &_
			whereSql &_
	...
	public function SelectByField(fieldName, value)
		...
		selectSQL = "" &_
		" SELECT " &_
			"  ROW_NUMBER() OVER (order by [Devices].[No] DESC) AS RowNum " &_
			" ,count(*) over () as [tcount] " &_
			" ,[Devices].* " &_
			" ,[DevicesMenu].Name AS MenuName " &_
			" ,( CASE [Devices].AdminNo WHEN 0 THEN [User].[Id] ELSE [Admin].[Id] END ) AS UserId " &_
		...
		" FROM [Devices] AS [Devices] " &_
		" LEFT JOIN [DevicesMenu] AS [DevicesMenu] ON([Devices].MenuNo = [DevicesMenu].No) " &_
		" LEFT JOIN [User] AS [User] ON([Devices].UserNo = [User].No) " &_
		" LEFT JOIN [Admin] AS [Admin] ON([Devices].AdminNo = [Admin].No) " &_
		" WHERE [Devices]." & fieldName & " = ? "
	...
	public function Update(obj)
		Dim strSQL
		strSQL= "Update [Devices] set [MenuNo] = ?, [Name] = ?, [Contents1] = ?, [Contents2] = ?, [Contents3] = ?,[Images1]=?,[Images2]=?,[Images3]=?,[Images4]=?,[State]=?,[ImagesList]=? Where No = ? "
		set objCommand=Server.CreateObject("ADODB.command") 
		objCommand.ActiveConnection=DbOpenConnection()
		objCommand.NamedParameters = False
		objCommand.CommandText = strSQL
 	...
 	public function Delete(No)
		...
		" DELETE FROM [Devices] WHERE No in(SELECT T_INT FROM @T); "
