		...
		<div class="inner">
			...
			<div class="sub_description">
						...
						<td colspan="2" style="margin:0px;padding:0px;border-bottom:0px;">
							...
							<%For each files in BoardFilesModel.Items%>
								<div class="row">
									...
									<button type="button" class="white" onclick="location.href='/Utils/download.asp?pach=/upload/DnABoard/&file=<%=files.Name%>';">다운로드</button>
								</div>
							<%next%>
							</div>
						</td>
				...
				<div style="text-align:right;">
					...
					<%if Model.AdminNo = Session("UserNo") And Model.AdminFg = 0 then%>
					...
					<button type="button" class="white mini" onclick="location.href='<%=ViewData("ActionReply")%>';">답변</button>
					<%end if%>
				</div>
