			...
			<div class="Detail_full">
				<div class="rows">
					<div class="area_images">
						<div class="big_img">
							<img class="trick"><img src="<%=iif( IsNothing(Images) or Images="","/images/bg_no_image.png","/upload/Devices/"&Images )%>" id="big_img">
						</div>
						<div class="small_img">
							<ul>
								<%if Not(IsNothing(Model.Images1)) then %>
								<li><a href="#" class="active"><img class="trick"><img src="<%="/upload/Devices/"& Model.Images1%>" class="thumbnail"></a></li>
								<%end if%>
								<%if Not(IsNothing(Model.Images2)) then %>
								<li><a href="#"><img class="trick"><img src="<%="/upload/Devices/"& Model.Images2%>" class="thumbnail"></a></li>
								<%end if%>
								<%if Not(IsNothing(Model.Images3)) then %>
								<li><a href="#"><img class="trick"><img src="<%="/upload/Devices/"& Model.Images3%>" class="thumbnail"></a></li>
								<%end if%>
								<%if Not(IsNothing(Model.Images4)) then %>
								<li><a href="#"><img class="trick"><img src="<%="/upload/Devices/"& Model.Images4%>" class="thumbnail"></a></li>
								<%end if%>
							</ul>
						</div>
				...
				<div class="rows">
					<div class="area_description">
						<h2 class="title">Device Information</h2>
				...
				<div class="rows">
					...
					<div class="area_files">
						...
						<div class="row">
							...
							<button type="button" class="white" onclick="location.href='/Utils/download.asp?pach=/upload/Devices/&file=<%=obj.Name%>';">다운로드</button>
						</div>
						<%next%>
					</div>
				</div>
				...
				<div class="rows" style="border-bottom:0px solid;">
					<div class="area_related" id="slider1">
						<h2 class="title">Related Apps</h2>
						<div class="caption">
							...
							<div class="items" >
								<div class="list">
									<ul id="sliderList1">
										...
										<li>
											<div class="item">
											<a href="?controller=DevicesApps&action=AppsDetail&No=<%=obj.ProductNo%>">
												<div class="photo"><img class="trick"><img src="<%=iif( IsNothing(Images) or Images="","/images/bg_no_image.png","/upload/Apps/"&Images )%>" ></div>
												<div class="label">
													<p>
														<b><%=obj.ProductName%></b>
						...
						<div class="lists">
							...
							<div class="bbs_list">
								...
								<div style="margin-top:30px;text-align:right;" id="ActionRegiste">
									<button class="blue mini" onclick="location.href='?controller=Mypage&action=Qna&mode=Registe&Code=Devices&ProductNo=<%=No%>';">질문하기</button>
								</div>
...
function callList(Types,pageNo){
	...
	$.ajax({
		url : "?controller=DevicesAppsBoard&action=AjaxList&Code=Devices&Types="+Types+"&ProductNo="+No+"&Title="+Title+"&pageNo="+pageNo+"&partial=True",
		type : "GET",
